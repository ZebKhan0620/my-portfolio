...
export const HeroSection = () => {
  ...
  // Function to generate responsive orbits
  const getResponsiveOrbits = () => {
    if (!isVisible) return null;
    
    const getResponsiveSize = (baseSize: number) => {
      if (width && width < 480) {
        return Math.round(baseSize * 0.45);
  ...
  return (
    ...
    >
      ...
      >
        {t('hero.skipToContent') || 'Skip to main content'}
      </a>
      ...
      <div id="content" className="container relative z-10 px-4 sm:px-0 mx-auto">
        <div className="flex flex-col items-center">
          ...
          >
            <h1 className={`font-[var(--font-serif)] text-base xs:text-lg sm:text-xl md:text-3xl lg:text-4xl xl:text-5xl text-center mt-3 xs:mt-4 sm:mt-6 md:mt-8 tracking-wide font-medium ${
              language === 'ja' ? 'font-[var(--font-jp)]' : ''
            }`}>
              {t('hero.title')}
            </h1>
            <h2 className={`font-[var(--font-serif)] text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl text-center mt-1 xs:mt-2 tracking-wide text-gradient font-medium ${
              language === 'ja' ? 'font-[var(--font-jp)]' : ''
            }`}>
              {t('hero.subtitle')}
            </h2>
          ...
          >
            ...
            >
              ...
              <span className="font-semibold">{t('hero.letsConnect')}</span>
            </Link>
          </motion.div>
        </div>
        
        {/* Scroll indicator */}
        <div className="absolute bottom-[-130px] left-0 right-0 flex justify-center z-10 hidden xs:flex sm:hidden md:hidden lg:flex">
          <motion.div 
